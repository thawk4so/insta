...
$query = "SELECT * FROM intest_keep WHERE iaccount_id = '0' OR iaccount_id IS NULL OR iaccount_id = '' ORDER BY id ASC";
$stmt = $pdb->prepare($query);
$stmt->execute();
$results = $stmt->fetchAll(PDO::FETCH_ASSOC); 
foreach ($results as $iaccount) { 
    echo $iaccount['id']. " : ". $iaccount['name']."\n";
    ...
    $update_query = "UPDATE intest_keep SET iaccount_id = '{$instagram_account_id}' WHERE id = {$iaccount['id']}"; 
    //echo $update_query."\n";          
